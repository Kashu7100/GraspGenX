...
"""
Inference performance benchmark for GraspGenX.

Loads the model and a test point cloud, benchmarks inference across
acceleration modes (None / torch.compile), and reports a comparison table
with latency, frequency, and speedup.
...
Usage:
    ...
    # Custom mesh:
    pytest tests/test_inference_perf.py -v -s --mesh /path/to/object.obj
...
Benchmark parameters:
    ...
    Backbone         = ptv3vanilla
    Gripper cond     = z_offset
    Num grasps       = 100
    Diffusion steps  = 10
"""
...
def mesh_path(request):
    """Resolve the mesh path from --mesh or generate a random point cloud."""
...
def test_inference_perf(backbone, point_cloud_from_mesh, mesh_path):
    ...
    For each mode on the given backbone:
      1. Build / transform the model with z_offset gripper conditioning.
    ...
    mesh_name = mesh_path.name if mesh_path else "random_point_cloud"
    _print_table(backbone, mesh_name, rows)
